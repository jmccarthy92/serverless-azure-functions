...
var Utils = /** @class */ (function () {
                ...
                switch (_b.label) {
                    ...
                    case 1:
                        ...
                        for (eventsIndex = 0; eventsIndex < events.length; eventsIndex++) {
                            ...
                            if (eventsIndex === 0 &&
                                !constants_1.constants.durableFunctionTriggers.includes(bindingType)) {
                            ...
                            if (bindingType.includes(constants_1.constants.queue) &&
                                functionObject["events"][eventsIndex].queue) {
                                bindingUserSettings[constants_1.constants.queueName] =
                                    functionObject["events"][eventsIndex].queue;
                            }
    ...
    Utils.getEntryPointAndHandlerPath = function (handler, config) {
        ...
        var entryPoint = handlerSplit.length > 1
    ...
    Utils.spawnLocal = function (options) {
        ...
        var serviceDir = process.env.SLS_SERVICE_DIR || serverless.config.servicePath;
        var localCommand = path_1.join(serviceDir, "node_modules", ".bin", command);
